...
const ElevenLabsTestButton: React.FC = () => {
  ...
  const testElevenLabs = async () => {
    ...
    try {
      ...
      const { error, data } = await supabase.functions.invoke('test-elevenlabs-voices');
      ...
      setTestResult({
        ...
        voicesCount: data.voicesCount,
        audioSize: data.ttsTest?.audioSize
      });
      
      if (data.success) {
        toast.success(`ElevenLabs test successful! Found ${data.voicesCount} voices.`);
      } else {
        toast.error(`ElevenLabs test failed: ${data.error}`);
      }
